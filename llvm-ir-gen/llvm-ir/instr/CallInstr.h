...
public:
    ...
    string toMipsString_stack(string ident) override {
        ...
        for (int i = 0; i < uses->size(); ++i) {
            ...
            s += "\tsw $t0, -" + to_string(POSMAPSIZE.at(function->getIdent()) + 4 - tarPos) + "($t7)\n";
        }
        s += "\tsubi $t7, $t7, 4\n";
        s += "\tsw $ra, 0($t7)\n";
        s += "\tsubi $t7, $t7 " + to_string(POSMAPSIZE.at(function->getIdent())) + "\n";
        s += "\tjal " + function->getEntry()->getLabel() + "\n";
        s += "\taddi $t7, $t7 " + to_string(POSMAPSIZE.at(function->getIdent())) + "\n";
        s += "\tlw $ra, 0($t7)\n";
        s += "\taddi $t7, $t7, 4\n";
        if(function->getRetType() != FuncType::VOID) {
